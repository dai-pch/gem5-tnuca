# modified by yuanqing based on original cpu2000.py
# 2017.3.19
...
spec_dist = os.environ.get('M5_CPU2000', '/home/wubi/gem5/benchmark/cpu2000')
...
class Benchmark(object):
    def __init__(self, isa, os, input_set):
        ...
        if not hasattr(self.__class__, 'number'):
            self.number = self.__class__.__number__
        ...
        executable = joinpath(spec_dist, 'binaries', isa, os, str(self.number) + '.' + self.name, self.binary)
        if not isfile(executable):
            raise AttributeError, '%s not found' % executable
        self.executable = executable

        # root of tree for input & output data files
        data_dir = joinpath(spec_dist, 'data', str(self.number) + '.' + self.name)
